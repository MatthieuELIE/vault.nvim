...
- Global keymaps are now opt-in via `opts.keys`: omit it and none are set,
  only the `:Vault*` commands are registered. `opts.keys = true` applies the
  previous defaults as-is; a table opts in with per-key overrides.
  Buffer-local keymaps (`toggle_checkbox`, `archive_todos`, active only
  inside vault-managed `todos.md` buffers) remain default-on regardless.

### Fixed

- Warn via `vim.notify` when `templates_path` is configured but the resolved
  template file is unreadable, instead of silently seeding an empty note.

## [0.2.1] - 2026-07-27

### Fixed

- `quick_add_todo` now routes through the same open-buffer path as the other
  toggles, and reports save/write errors via `vim.notify` instead of failing
  silently.
