...
M.setup = function(opts)
    ...
    local buffer_local_keys = vim.tbl_extend('force', {
    ...
    if buffer_local_keys.toggle_checkbox or buffer_local_keys.archive_todos then
        vim.api.nvim_create_autocmd('BufEnter', {
            pattern = config.state.vault_path .. '/*/todos.md',
            callback = function(args)
                for _, spec in ipairs(buffer_local_keymap_specs) do
                    if buffer_local_keys[spec.name] then
                        vim.keymap.set(
                            'n',
                            buffer_local_keys[spec.name],
                            spec.fn,
